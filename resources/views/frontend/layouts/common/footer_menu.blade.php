						...
						@foreach(getAppStoreLinkFrontEnd() as $app)
							@if (!empty($app->logo))
								<a href="{{ $app->link }}" target="_blank">
									<img class="{{ $app->company == 'Apple' ?  'ms-3 ml-r12' : '' }}app-imgs" src="{{ url('/uploads/app-store-logos/thumb/'.$app->logo) }}" alt="{{ __('playstore') }}">
								</a>
							@else
								<a href="javascript:void(0)">
									<img class="app-imgs" src="{{ url('/uploads/app-store-logos/default-logo.jpg') }}" alt="{{ __('playstore') }}">
								</a>
							@endif
						@endforeach
