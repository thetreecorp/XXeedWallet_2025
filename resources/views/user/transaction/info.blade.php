@foreach($transactions as $key=>$transaction)
    @php
        $transactionTypes = [Deposit, Withdrawal, Transferred, Received, Exchange_From, Exchange_To, Request_Sent, Request_Received, Payment_Sent, Payment_Received, Crypto_Sent, Crypto_Received, Unlock_Kemecoin100];
                ...
                <div class="deposit-circle d-flex justify-content-center align-items-center">
                    @php
                        if (in_array($transaction->transaction_type?->id, [Deposit, Withdrawal])) {
                         ...
                         else if (in_array($transaction->transaction_type?->id, [Unlock_Kemecoin100])) {
                            $transactionImage = $transaction->end_user?->picture ?? null;
                            $directoryName = 'party';
                        } else {
                            $transactionImage = null;
            ...
            <div class="d-flex justify-content-center align-items-center">
                <div>
                    <p class="mb-0 gilroy-medium text-gray-100 r-f-12 f-16 ph-20">
                        @if (in_array($transaction->transaction_type?->id, [Deposit, Received, Exchange_To, Payment_Received, Request_Sent, Crypto_Received, Unlock_Kemecoin100])) 
