...
if ($('#crypto-front-initiate').length) {
    ...
    function setToCurrencies(response) {
        ...
        if (response.length || response.directionCurrencies) {
    ...
    function setFromCurrenciesType(response) {
        ...
        if (response[0]) {
            let first = response[0].from_currency;
    ...
    function getCurrenciesByType(directionType) {
        ...
        if (directionType) {
            ...
            .done(function (response) {
                if (directionType == 'crypto_sell' && response.toCurrencies == '') {
                    var modifiedStr = directionType.replace(/_/g, ' ').replace(/\b\w/g, function (c) {
                        return c.toUpperCase();
                    });

                    $('.send_amount_error').addClass('error').text(modifiedStr + ' ' + directionNotAvaillable);
                    $('.dot').removeClass('display-hide');
                    $('.dot-message').addClass('h-9p');
                    $('#crypto_buy_sell_button').attr('disabled', true);
                } else {
                    $("#send_amount").val(response.minAmount);
