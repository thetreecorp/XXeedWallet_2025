...
<div class="bg-white pxy-62 pt-62 custom-input-height shadow" id="crypto_exchange_details">
    <p class="mb-0 f-26 gilroy-Semibold text-center">{{ __(ucwords(str_replace('_', ' ', $transInfo['exchangeType']))) }}</p>
    <p class="mb-0 text-center f-13 gilroy-medium text-gray mt-4 dark-A0">{{ __('Step: 2 of 3') }}</p>
    <p class="mb-0 text-center f-18 gilroy-medium text-dark dark-5B mt-2">{{ __('Confirm :x', ['x' => __(ucwords(str_replace('crypto_', '', $transInfo['exchangeType'])))]) }}</p>
    <div class="text-center">{!! svgIcons('stepper_confirm') !!}</div>

    <form action="{{ route('user_dashboard.crypto_buy_sell.success')}}" method="post" id="crypto_buy_sell_from" enctype="multipart/form-data">
        @csrf
        ...
        <div class="mt-32">
            <div class="exchange-send-get d-flex justify-content-between">
                ...
                <div class="send-left-box w-50">
                    ...
                    <p class="mb-0 f-24 l gilroy-Semibold mt-1"><span class="text-dark">{{  formatNumber($transInfo['defaultAmnt'], $transInfo['from_currency']) }}</span><span class="text-primary"> {{ $transInfo['fromCurrCode'] }}</span></p>
                    
                    <p class="mb-0 f-12 leading-17 gilroy-medium text-gray mt-1"><span>{{ __('Fees') }} ≈ </span><span>{{ formatNumber($transInfo['totalFees'], $transInfo['from_currency']) }} {{ $transInfo['fromCurrCode'] }}</span></p>
                ...
                <div class="get-right-box w-50">
                    ...
                    <p class="mb-0 f-24 l gilroy-Semibold mt-1"><span class="text-white">{{  formatNumber($transInfo['getAmount'], $transInfo['to_currency']) }}</span><span class="text-warning"> {{ $transInfo['toCurrCode'] }}</span></p>
                    
                    <p class="mb-0 f-12 leading-17 gilroy-medium text-muted mt-1"><span>1 {{ $transInfo['fromCurrCode'] }} ≈ {{  formatNumber($transInfo['dCurrencyRate'], $transInfo['to_currency']) }} {{ $transInfo['toCurrCode'] }}</span></p>
        ...
        @if($transInfo['exchangeType'] !== 'crypto_buy')
            ...
            <div class="param-ref sm-send mt-20">
                ...
                <select class="select2" data-minimum-results-for-search="Infinity" name="pay_with" id="pay_with">
                    <option value="others">{{ __(':x Address', ['x' => $transInfo['fromCurrCode']]) }}</option>
                    @if($transInfo['fromWallet'])
                        <option value="wallet">{{ __('Wallet') }} ({{ $transInfo['fromWallet']->balance }} {{ $transInfo['fromCurrCode'] }})</option>
                    @endif
                </select>
            </div>
            ...
            <div class="d-flex flex-wrap justify-content-between">
                <div class="left-merchant pt-20p">
                    ...
                    <p class="mb-0 f-32 l gilroy-Semibold mt-2"><span class="text-dark">{{  formatNumber($transInfo['finalAmount'], $transInfo['from_currency']) }}</span><span class="text-primary"> {{ $transInfo['fromCurrCode'] }}</span></p>
                ...
                <div class="right-qr-code mt-24 user-profile-qr-code">
                    <img class="img-fluid" src="{{ qrGenerate($transInfo['merchantAddress']) }}" alt="image" width="170" height="170">
            ...
            <div class="d-flex position-relative copy-div">
                <input class="form-control input-form-control apply-bg" type="text" id="merchantAddress" value="{{ $transInfo['merchantAddress'] }}" name="merchant_address" readonly>
        ...
        <div class="param-ref sm-send mt-20">
            ...
            <select class="select2" data-minimum-results-for-search="Infinity" name="receive_with" id="receive_with">
                <option value="address">{{ __(':x Address', ['x' => $transInfo['toCurrCode']]) }}</option>
                <option value="wallet">{{ __(':x Wallet', ['x' => $transInfo['toCurrCode']]) }}  </option>
            </select>
        </div>
        ...
        <div id="crypto_address_section">
            <div class="label-top mt-20">
                ...
                <input type="text" class="form-control input-form-control apply-bg crypto_address" name="crypto_address" id="crypto_address" required data-value-missing="{{ __('This field is required.') }}" placeholder="{{ __('Please provide your :x address', ['x' => $transInfo['toCurrCode']]) }}">
        ...
        <div class="d-grid">
            <button type="submit" class="btn btn-lg btn-primary mt-4 submit-button" id="exchange-confirm-submit-btn">
                ...
                <span id="cryptoExchangeConfirmBtnText">{{ __('Confirm & :x', ['x' => __(ucwords(str_replace('crypto_', '', $transInfo['exchangeType'])))]) }}</span>
...
<script type="text/javascript">
    ...
    var expireSec = "{{ $transInfo['expire_seconds'] }}";
