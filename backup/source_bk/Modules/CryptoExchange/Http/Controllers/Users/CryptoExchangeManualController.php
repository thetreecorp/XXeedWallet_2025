...
class CryptoExchangeManualController extends Controller
{
    ...
    public function exchangeList()
    {
        ...
        $cryptoExchanges =  $this->cryptoExchange->getExchangesList($data['from'], $data['to'], $data['status'], 'all', $data['user']);
    ...
    public function exchange()
    {
        ...
        if (count($data['cryptoDirections'])) {
            ...
            $data['minLimit'] = ($direction->min_amount != 0 ) ? cryptoFormat($direction->min_amount) : cryptoFormat(1);
            // Reciving currencies
    ...
    public function exchangeOfCurrency(CryptoExchangeRequest $request)
    {
        ...
        $data['transInfo']['expire_seconds'] = $expireSec = 60;
        $expireTime = date("F d, Y h:i:s A", strtotime('+'.$expireSec.' seconds'));
    ...
    {
        date_default_timezone_set(preference('dflt_timezone'));
        $nowTime = date("F d, Y h:i:s A", strtotime("now"));
        $expireTime = session('paymentInfo')['transInfo']['expire_time'];

        $paymentInfo = session('paymentInfo');
        if (empty($paymentInfo) || $nowTime > $expireTime) {
            return redirect()->route('user_dashboard.crypto_buy_sell.create');
        }
        $data = $paymentInfo;
    ...
    public function exchangeOfCurrencyConfirm(CryptoUserRequest $request)
    {
        ...
        //Admin Notification
        $notificationToAdmin = (new \Modules\Cryptoexchange\Services\Mail\NotifyAdminOnCryptoMailService)->send($response['cryptoExchange'], ['type' => 'crypto-exchange', 'medium' => 'email']);
        $data = [];
        ...
        $data['transInfo']['trackUrl']    = url('crypto-exchange/track-transaction', $uuid);
        Session::forget('transInfo');
        clearActionSession();
