...
class CryptoExchangeController extends Controller
{
    ...
    public function cryptoExchange()
    {
        ...
        $cryptoDirections = (new ExchangeDirection)->exchangeDirectionWithCurrency($type, 'from_currency_id', []);
        $currencyId = optional($cryptoDirections->first())->from_currency_id;
        $toCurrencies = ExchangeDirection::getCurrencies($currencyId, $type);

        $data = [
            'menu' => $menu,
            'pref' => $pref,
            'exchange_type' => $type,
            'cryptoDirections' => $cryptoDirections,
    ...
    public function getTabDirection()
    {
        ...
        return response()->json([
            ...
            'minAmount' => $fromCurrencies[0]->min_amount,
            'status' => $status
        ]);
    }
    ...
    public function cryptoBuySellSuccess(CryptoBuySellSuccessRequest $request)
    {
        ...
        $data['transInfo']['trackUrl'] = url('crypto-exchange/track-transaction', $cryptoExchange['uuid']);
        $this->sessionForget();
        clearActionSession();
    ...
    public function cryptoBuySellPayment(CryptoBuySellSuccessRequest $request)
    {
        ...
        if ($method == Payeer && !empty($currencyPaymentMethod)) {
            ...
            $data['m_orderid']  = $m_orderid  = otpCode6();
