...
class LoginController extends Controller
{
    ...
    public function authenticate(Request $request)
    {
        ...
        if (!empty($loginData['value'])) {
            ...
            $checkUserMobile = $this->checkUserMobilCheck($loginData['value']);
            //dd($loginData['value']);
            if($checkUserMobile) {
               
                return redirect('/register/verify-phone')->with('status', __('Please verify your phone number to log in'));
            }
    ...
    //Check User Verification Status
    protected function checkUserMobilCheck($email)
    {
        $user = User::where(['email' => $email])->first(['id', 'first_name', 'last_name', 'email', 'status', 'mobile_check_status']);
        if ($user && $user->mobile_check_status != 1) {
            return 1;
        }
        return 0;
    }
