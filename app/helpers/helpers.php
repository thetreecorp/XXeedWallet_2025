...
if (!function_exists('unlockAndTransferCoins')) {
    function unlockAndTransferCoins($currencyId, $transferCurrencyId)
    {
        $currentDate = now();

        try {
            // Lấy tất cả các giao dịch thỏa mãn điều kiện
            $transactions = Transaction::where('currency_id', $currencyId)
                ->whereRaw('DATE_ADD(created_at, INTERVAL 100 DAY) <= ?', [$currentDate])
                ->whereRaw("(note != 'released' OR note IS NULL OR note = '')")
                ->get();
            ...
            // Nhóm các giao dịch theo user_id và currency_id
            $groupedTransactions = $transactions->groupBy(['user_id', 'currency_id']);

            foreach ($groupedTransactions as $userId => $currencyGroups) {
                foreach ($currencyGroups as $currencyId => $transactionsGroup) {
                    // Tính tổng total của nhóm
                    $totalAmount = $transactionsGroup->sum('total');

                    DB::beginTransaction(); // Bắt đầu transaction

                    try {
                        // Cập nhật note cho tất cả các giao dịch trong nhóm
                        $transactionIds = $transactionsGroup->pluck('id')->toArray();
                        ...
                        $newTransaction->total = $totalAmount; // Giá trị dương
                        $newTransaction->subtotal = $totalAmount; // Giá trị dương
                        $newTransaction->transaction_type_id = 16; // Loại giao dịch
                        ...
                        // Update wallet
                        Wallet::incrementWalletBalance($userId, $transferCurrencyId, floatval($totalAmount));
                        Wallet::deductAmountFromWallet($userId, $currencyId, floatval($totalAmount));

                        DB::commit(); // Commit transaction nếu mọi thứ thành công
                    } catch (\Exception $e) {
                        DB::rollBack(); // Rollback transaction nếu có lỗi
                        Log::error('Error processing transaction for user_id: ' . $userId, [
                            'error' => $e->getMessage(),
                            'trace' => $e->getTraceAsString(),
                        ]);
                        throw $e; // Ném lại exception để xử lý tiếp nếu cần
                    }
                }
            }
        } catch (\Exception $e) {
            Log::error('Error in unlockAndTransferCoins function', [
                'error' => $e->getMessage(),
                'trace' => $e->getTraceAsString(),
            ]);
            throw $e; // Ném lại exception để xử lý tiếp nếu cần
...
if (!function_exists('createSsoUrl')) {
    function createSsoUrl($type) {
        $redirectUriBase64 = base64_encode(env('REDIRECT_URI'));
        if ($type === 'login') {
          $ssoUrl = env('SSO_LOGIN_URL');
        } elseif ($type === 'register') {
          $ssoUrl = env('SSO_REGISTER_URL');
        } else {
          throw new Exception("Invalid type: $type");
        }
        return "$ssoUrl?redirect_uri=$redirectUriBase64";
    }
}

if (!function_exists('createrRibanoUrl')) {
    function createrRibanoUrl($type) {
        $redirectUriBase64 = base64_encode(env('REDIRECT_URI'));
        if ($type === 'login') {
          $url = env('RIBANO_LOGIN_URL');
        } elseif ($type === 'register') {
          $url = env('RIBANO_REGISTER_URL');
        } else {
          throw new Exception("Invalid type: $type");
        }
        return "$url?redirect_uri=$redirectUriBase64";
    }
}
...
if (!function_exists('deEncryptToken')) {
...
if (!function_exists('sendEmail')) {
    function sendEmail($type, $email) {
        try {
            if($type == 'email-otp')
                Mail::to($email)->send(new EmailOtp($email));
            else
                Mail::to($email)->send(new EmailOtp($email));
            return 1;
...
if (! function_exists('getPaymoneySettings')) {
    ...
    function getPaymoneySettings($type = null)
    {
        ...
        $array = [
            'transaction_types' => [
                'web' => [
                    'sent' => [Deposit, Transferred, Exchange_From, Exchange_To, Request_Sent, Withdrawal, Payment_Sent, Paymob],
            ...
            'payment_methods' => [
                'web' => [
                    'all' => [Mts, Stripe, Paypal, PayUmoney, Bank, Coinpayments, Payeer, Crypto, Coinbase, Paymob],
                    'deposit' => [Mts, Stripe, Paypal, PayUmoney, Bank, Coinpayments, Payeer, Coinbase, Paymob],
