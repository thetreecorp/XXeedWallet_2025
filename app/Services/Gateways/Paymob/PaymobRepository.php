...
class PaymobRepository
{
    ...
    public function getPayableLink($authSessionKey, $amount, $methodId, $currencyId, $currency, $paymobPayment, $type = 'card', $reference = '')
    {
        Log::critical('Paymob Log 1',['data'=>['auth_session'=>$authSessionKey]]);
        if($authSessionKey){
            Session::put('paymob', ['transInfo'=>['amount'=>$amount, 'totalAmount'=>$amount, 'payment_method_id'=>$methodId,'currency_id'=>$currencyId, 'currency'=>$currency, 'uuid'=>$reference]]);

            $referenceId = ($reference == '') ? $this->getPaymobRefNo($currencyId,$methodId) : $reference;
            Log::critical('Paymob Log 2',['data'=>['reference_no'=>$referenceId]]);
            ...
            Log::critical('Paymob Log 3',['data'=>['data_to_api'=>$data]]);
            ...
            Log::critical('Paymob Log 4',['data'=>['payment_url'=>$url]]);
...
//    }
    private function paymobCardCheckout($sessionKey, array $data){
        ...
        curl_close($curl);
        $resCode = curl_getinfo($curl, CURLINFO_HTTP_CODE);
        Log::critical('Paymob Log 5',['data'=>['card_response'=>$response, 'res_code'=>$resCode]]);
            ...
            try{
                if($authKey = $this->getPaymobAuthKey()){
                    ...
                    if($resCode == 200){
                        ...
                        $this->updateTransactionReferenceId($paymobPayment->reference_id,$res->id);
                        $paymobPayment->save();

                        $sessionValue      = session('paymob.transInfo');
                        $amount            = (double) $sessionValue['amount'];
                        $payment_method_id = $paymobPayment->method_id;
