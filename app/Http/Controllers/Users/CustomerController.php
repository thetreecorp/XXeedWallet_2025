...
class CustomerController extends Controller
{
    ...
    public function dashboard()
    {
        ...
        $unlockTokenId = findCurrencyBySymbol($this->keme100);
        $targetIdtokenReleased = findCurrencyBySymbol($this->keme0);
        ...
        // Count lock kemecoin100
        $result = $this->helper->getCurrencyIdsBySymbols([$this->keme100]);

        if ($result['success']) {
            $currencies = $result['data'];
            $data['calculateFunds'] = $this->helper->calculateFunds(Auth::user()->id,  $currencies);
        }
    ...
    public function getWallets()
    {
        ...
        /* CUSTOM CREATE NEAR WALLET */
        $find_wallet = UserWallet::where('user_id', Auth::user()->id)->first();
        if(!$find_wallet) {
            $randomString = Str::random(12, 'abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789');

            $create_sub_account_url = env('NEAR_API_URL') ? env('NEAR_API_URL') . 'create_normal_sub_account' : 'http://162.19.248.57:7777/create_normal_sub_account' ;

            $response = Http::withHeaders([
                'Accept' =>  'application/json',
                'Content-Type' => 'application/json'
            ])->post($create_sub_account_url, [
                'name' => strtolower($randomString),
            ]);
            ...
            if($create_account['status'] == 400) {
                ...
                exit('Create near sub account failed');
            }
            else {
                $data = $response->json();

                $create_wallet = UserWallet::create([
                    'wallet_address' => strtolower($randomString) . '.' . (env('MASTER_ACCOUNT') ? env('MASTER_ACCOUNT') : 'ctoken.testnet'),
                    'user_id' => Auth::user()->id,
                    'type' => 'near',
                    'status' => 'active',
            ...
            $data['near_wallet'] = strtolower($randomString) . '.' . (env('MASTER_ACCOUNT') ? env('MASTER_ACCOUNT') : 'ctoken.testnet');


        }
        else {
            $data['near_wallet'] = $find_wallet->wallet_address;
        }
        /* END CUSTOM CREATE NEAR WALLET */
